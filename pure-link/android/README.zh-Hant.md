...
支援全形 `：`、全形空白和前後空白。手動輸入時，額外支援單獨合法的裸 slug，例如 `A3cd8`。`論文 PureLink: A3cd8` 中的「論文」只在本機介面作為顯示提示，不會改變 slug 或產生的網址。
...
- 選取／分享的文字會解析後立即從輸入框清除；目前畫面只保留候選項目的本機標籤與 slug，不會存進偏好設定、資料庫或剪貼簿歷程。
