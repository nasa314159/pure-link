...
5. 可選擇**編輯說明**；它會開啟使用 Android 一般系統鍵盤（Samsung Keyboard、Gboard、語音輸入、emoji 與已安裝 IME）的私有小型編輯器。這次性說明最多 280 個 Unicode code points。
6. 按 **分享**：
...
- 輕量頂部工具列只保留剪貼簿、分享與帳號；Globe 只保留在底部鍵盤列。帳號只會在瀏覽器開啟相符語系的 PureLink 帳號頁，不會建立原生帳號狀態。
- 候選控制列先放圖示式的**刪除已選候選項目**，接著是全選與 `+ 全部`。它絕不清除未選候選或說明；刪除全部候選時才回到手動模式。
- 緊湊說明預覽可開啟**編輯說明**；仍保留只貼入說明的獨立貼上動作，絕不把說明文字解析成候選項目。
- 保持來源順序的候選列；多個候選時提供全選與 `+ 全部`。
- 只建立 `https://no-no.uk/<slug>` 和 `https://no-no.uk/<slug>+` 的開啟／預覽動作。
- Android 系統分享選單與底部列的鍵盤切換按鈕。
...
Android 開啟系統分享選單，不代表接收者已接受或送出訊息。PureLink 因此會保留工作階段（包括回傳的小卡網址）直到 IME 工作階段結束。若修改已選候選，舊小卡網址代表不同的分享選取，因此會被清除。
