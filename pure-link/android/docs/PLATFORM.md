...
Candidate management starts with an accessible icon-only **Delete selected candidates** control, then Select All and `+ All`, with the remaining row width intentionally empty. It removes only selected rows, preserves the unselected rows’ order and `+` state, and preserves the description while a candidate remains. Deleting the final candidate returns to Manual mode. The compact description preview opens a private `DescriptionEditorActivity`; this standard multiline `EditText` uses Android’s normal selected system IME and supports arbitrary Unicode, including Traditional Chinese, Japanese, emoji, and multiline text, capped by the existing 280-code-point filter. The editor has no network access and returns text only to the active IME session.
...
7. Use Edit description and confirm a private, focused multiline editor opens with the device’s normal system keyboard. Test Traditional Chinese, Japanese, emoji, and 280 code points; Done must return the exact bounded text, while Cancel keeps the original description, candidates, selections, and `+` states. The compact IME preview remains at one or two lines.
