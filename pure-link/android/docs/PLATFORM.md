...
The keyboard opens in Manual mode with its internal slug field visible and focused. Its key events target only that slug field or the optional description field, never the host editor. The dark five-row layout exposes exactly `[A-Za-z0-9_-]`: digits, QWERTY, ASDF, Shift/ZXCVBNM/Backspace, then Globe/`_`/`-`/Enter. Shift has lowercase, one-shot, and double-tap Caps Lock states; only a typed letter consumes one-shot Shift. The five direct toolbar icons are Clipboard, Manual, Globe, Share, and Account; Account only opens the localized web account/login entry.
