...
- [ ] 在 Lemon Squeezy 與 ECPay Test Mode 建立 `small`／`standard`／`large` 公開方案，設定測試密鑰與通知端點。
- [ ] 驗證付款、重送、累積部分退款、全額退款、錯誤金額／商店／商家與簽名事件，以及確認後額度只交付到發起購買的帳號。
- [ ] 付款供應商審核與正式整合完成後，再設定正式密鑰並啟用正式結帳。
- [ ] 只有在正式付款功能獲核准並完成測試後，才啟用正式收款設定。
- [ ] 正式 ECPay 僅在商家核准後設定 `PUBLIC_ORIGIN=https://no-no.uk`、`ECPAY_ENVIRONMENT=production` 與完整 secrets，並確認選到正式 V5 endpoint。
- [ ] 完成正式人工 QA：瀏覽器返回僅供說明，只有已驗證的伺服器端 ECPay 回呼才是額度交付權威。
