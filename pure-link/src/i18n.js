...
export function resolveLocale(request, routeLocale = null) {
  return getCookieLocale(request) || normalizeLocale(routeLocale) || localeFromAcceptLanguage(request.headers.get('accept-language')) || DEFAULT_LOCALE;
}
...
export function localeCookie(locale) {
  return `${LOCALE_COOKIE}=${normalizeLocale(locale) || DEFAULT_LOCALE}; Path=/; Max-Age=31536000; SameSite=Lax; Secure`;
}
...
export const messages = Object.freeze({
  en: {
    ...
    home: {
      title: 'PureLink — Privacy-friendly URL shortener and formula sharing', description: 'Shorten links without ads or needless tracking, share readable LaTeX formulas, and create quiet note cards with PureLink.',
  ...
  'zh-Hant': {
    ...
    home: {
      title: 'PureLink — 不追蹤的短網址、公式與小卡分享', description: '用 PureLink 建立不含廣告與不必要追蹤的短網址，分享清楚的 LaTeX 公式與安靜的小卡。',
